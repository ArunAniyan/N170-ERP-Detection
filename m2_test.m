...
function [in1 in2] = m2_test(coefs,thr,ts)
...
%% Sort Pairs - Based total energy of positve & negetive cofx
...
if (length(pos1) == 1)  %% Only if pair exist return.
...
g = gausswin(19);  % Gaussian Weight vector
...
[sds1 pos2] = sort(td,'descend');  % No need 
...
if (abs (td(1)) < abs(td(2)))
    
    if( pd(pos1(1)) > pd(pos1(2) ) )    % Removed pr1 and pd - Check
    ...
    if ( pd(pos2(1)) > pd(pos2(2)) )    %% Check if it has greatest coef
